...
async def _process_claude(message: Message, state: FSMContext, text: str):
    ...
    try:
        ...
        response, prompt_tokens, completion_tokens = await ask_claude(history, mode_prompt)
        ...
        await message.answer(
            ...
            reply_markup=chat_controls_keyboard("claude", mode),
            parse_mode="HTML",
        )
...
async def handle_claude_photo(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await thinking_msg.delete()
        await message.answer(escape(strip_markdown(response)) + footer, reply_markup=chat_controls_keyboard("claude", mode), parse_mode="HTML")
    except Exception as e:
        await log_usage(user_id, "claude", 0, 0, False)
        await thinking_msg.delete()
...
async def handle_claude_voice(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        return
    await _process_claude(message, state, text)
