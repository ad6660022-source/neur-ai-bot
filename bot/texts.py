...
def get_welcome_text(first_name: str, is_new: bool = False) -> str:
    ...
    return (
        ...
        f"🤖 Добро пожаловать в <b>NEUR AI</b> — три лучших нейросети в одном боте.\n"
        f"{trial_block}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🟢 <b>ChatGPT</b> (GPT-4o) — универсальный\n"
        f"🟣 <b>Claude</b> (Sonnet 4.6) — аналитик\n"
        f"🔵 <b>DeepSeek</b> (V3) — технический\n\n"
        f"👨‍💻 7 режимов работы: Программист, Копирайтер, Учитель...\n"
...
def get_ai_selection_text() -> str:
    ...
    lines.append("\n<i>После выбора модели выбери режим (Программист, Учитель и др.)</i>")
...
def get_plans_text() -> str:
    ...
    plans_info = {
        "free":  {"name": "Бесплатный",  "price": "0",                        "features": ["🟢 ChatGPT: 5/мес · 3/день", "🔵 DeepSeek: 10/мес · 5/день", "🟣 Claude: недоступен", "💾 Сохранение чатов: нет"]},
        "basic": {"name": "Basic",        "price": f"{PLAN_STARS['basic']} ⭐", "features": ["🟢 ChatGPT: 30/мес · 10/день", "🔵 DeepSeek: 50/мес · 20/день", "🟣 Claude: недоступен", "💾 Сохранение чатов: 3"]},
        "pro":   {"name": "Pro",          "price": f"{PLAN_STARS['pro']} ⭐",   "features": ["🟢 ChatGPT: 80/мес · 30/день", "🟣 Claude: 10/мес · 5/день", "🔵 DeepSeek: 150/мес · 60/день", "💾 Сохранение чатов: 20"]},
        "ultra": {"name": "Ultra",        "price": f"{PLAN_STARS['ultra']} ⭐", "features": ["🟢 ChatGPT: 200/мес · 80/день", "🟣 Claude: 30/мес · 15/день", "🔵 DeepSeek: ∞ безлимит", "💾 Сохранение чатов: ∞"]},
    }
...
def get_usage_text(sub, user=None) -> str:
    ...
    bonus_str = ""
    if user and getattr(user, "bonus_requests", 0) > 0:
        bonus_str = f"\n🎁 Бонусных запросов: <b>{user.bonus_requests}</b>"

    return (
        f"📊 <b>Ваш профиль</b>\n\n"
        f"{emoji} Тариф: <b>{plan.upper()}</b>{badge}{expires_str}{bonus_str}\n\n"
...
def get_referral_text(referral_code: str, bot_username: str, referred_count: int = 0) -> str:
    ...
    return (
        ...
        f"Приглашай друзей — получай <b>+10 бонусных запросов</b>\n"
        f"за каждого кто зарегистрируется по твоей ссылке!\n\n"
        f"🔗 Твоя ссылка:\n<code>{link}</code>\n\n"
        f"👥 Приглашено: <b>{referred_count}</b> друзей\n\n"
        f"<i>Бонусы расходуются автоматически до основного лимита.</i>"
    )
